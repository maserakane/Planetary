...
async function getDecay(planet, userAccount) {
    ...
    for (let endpoint of apiEndpointsLive) {
        try {
            const data = await apiRequestWithRetryh(endpoint, requestData, 1, 50);
            return data;
        } catch (error) {
...
// Nouvelle fonction pour additionner le VP et calculer le decay individuellement pour chaque planète
async function getTotalVotepowerAndLastVoteTime() {
    let totalVP = 0;
    let totalVPAfterDecay = 0;
    const userAccount = localStorage.getItem("userAccount");
    const planets = ['magor', 'kavian', 'eyeke','naron','neri','veles','velesunn','neriunn', 'naronunn', 'magorunn', 'kavianunn', 'eyekeunn'];
    let details = [];
    
    for (const planet of planets) {
        // VP pour cette planète
        let vp = 0;
        try {
        ...
        // Decay spécifique à cette planète
        let vote_time_stamp = null;
        try {
            const decayResponse = await getDecay(planet, userAccount);
            console.log(`[DEBUG] Réponse decay pour ${planet}:`, decayResponse);
            if (decayResponse.rows.length > 0 && decayResponse.rows[0].vote_time_stamp) {
                ...
                console.log(`[DEBUG] ${planet} - vote_time_stamp trouvé:`, vote_time_stamp);
            } else {
                console.log(`[DEBUG] ${planet} - Aucun vote_time_stamp trouvé. Nombre de rows:`, decayResponse.rows.length);
            }
        } catch (error) {
            console.error(`Erreur lors de la récupération du decay pour ${planet} :`, error);
        }
        
        // Calcul du decay pour cette planète spécifique
        let vpAfterDecay = vp;
        if (vp > 0 && vote_time_stamp) {
            const currentTime = new Date();
            const timeDifference = (currentTime - vote_time_stamp) / 1000;
            const fraction_s = timeDifference / 2629800;
            vpAfterDecay = vp / Math.pow(2, fraction_s);
        }
        
        totalVPAfterDecay += vpAfterDecay;
        details.push({ planet, vp, vote_time_stamp, vpAfterDecay });
    }
    
    // Affichage détaillé
    console.log('--- Détail VP par planète (avec decay individuel) ---');
    details.forEach(d => {
        console.log(`${d.planet} : VP initial=${d.vp}, VP après decay=${d.vpAfterDecay.toFixed(2)}, vote_time_stamp=${d.vote_time_stamp}`);
    });
    console.log('VP total initial (toutes planètes) :', totalVP);
    console.log('VP total après decay (toutes planètes) :', totalVPAfterDecay.toFixed(2));
    
    return { totalVP, totalVPAfterDecay, details };
}
...
async function fetchChest(landId) {
    try {
        ...
        if (responseData && responseData.rows && responseData.rows.length > 0) {
            ...
            // Utiliser le total VP (incluant les unn) avec decay calculé individuellement par planète
            const { totalVP, totalVPAfterDecay, details } = await getTotalVotepowerAndLastVoteTime();
            let voteafterdecay = totalVPAfterDecay;
            
            // Logs détaillés pour le calcul du decay
            console.log('=== CALCUL DU DECAY VP (par planète) ===');
            console.log('VP total initial (toutes planètes):', totalVP);
            console.log('VP total après decay (toutes planètes):', voteafterdecay.toFixed(2));
            console.log('Pourcentage de VP restant:', ((voteafterdecay / totalVP) * 100).toFixed(2) + '%');
            console.log('Détail par planète:');
            details.forEach(d => {
                if (d.vp > 0) {
                    const decayPercent = ((d.vpAfterDecay / d.vp) * 100).toFixed(2);
                    console.log(`  ${d.planet}: ${d.vp} → ${d.vpAfterDecay.toFixed(2)} (${decayPercent}% restant)`);
                }
            });
            console.log('==========================================');
    ...
    try {
        ...
        if (landData.length > 0) {
            ...
            for (const landId of landData) {
                try {
                    // Essayer plusieurs API en cas d'échec
                    let templateResponse;
                    const apiUrls = [
                        `https://atomic-wax.tacocrypto.io/atomicassets/v1/assets/${landId}`,
                        `https://atomic.3dkrender.com/atomicassets/v1/assets/${landId}`,
                        `https://atomic-api.wax.cryptolions.io/atomicassets/v1/assets/${landId}`,
                        `http://wax-atomic-api.eosphere.io/atomicassets/v1/assets/${landId}`,
                        `http://atomic.wax.eosrio.io/atomicassets/v1/assets/${landId}`,
                        `https://wax-aa.eu.eosamsterdam.net/atomicassets/v1/assets/${landId}`,
                        `https://aa.dapplica.io/atomicassets/v1/assets/${landId}`,
                        `https://atomic-wax.a-dex.xyz/atomicassets/v1/assets/${landId}`,
                        `https://wax-atomic.alcor.exchange/atomicassets/v1/assets/${landId}`,
                        `https://aa-wax-public1.neftyblocks.com/atomicassets/v1/assets/${landId}`,
                        `https://wax.eosusa.io/atomicassets/v1/assets/${landId}`,
                        `https://atomicassets.ledgerwise.io/atomicassets/v1/assets/${landId}`,
                        `https://atomic-wax.qaraqol.com/atomicassets/v1/assets/${landId}`,
                        `https://wax-atomic.eosiomadrid.io/atomicassets/v1/assets/${landId}`
                    ];
                    
                    for (const apiUrl of apiUrls) {
                        try {
                            templateResponse = await fetch(apiUrl);
                    ...
                    if (!templateResponse || !templateResponse.ok) {
                        throw new Error('All asset APIs failed');
                    }
