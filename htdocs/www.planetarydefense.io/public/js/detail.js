    ...
    fetch(`/mission/user-templates?userAccount=${userAccount}`)
        ...
        .then(data => {
            ...
            const templateIds = templates.flatMap(template => {
                return ['crews', 'faces', 'arms', 'lands', 'warlords', 'mercenaries'].flatMap(key => {
...
function calculateStats(detail, quantity, category) {
    ...
    if (category === 'crews' || category === 'arms') {
        ...
        totalDefense = 0;
        totalMoveCost = 0;
    } else if (category === 'warlords') {
        // Les warlords fournissent des slots supplémentaires
        totalAttack = 0;
        totalDefense = 0;
        totalMoveCost = 0;
        slots = 25; // Les warlords fournissent 25 slots chacun
    } else if (category === 'mercenaries') {
        // Les mercenaries ont leurs propres stats d'attaque et défense
        totalAttack = detail.attack || 0;
        totalDefense = detail.defense || 0;
        totalMoveCost = detail.movecost || 0;
        slots = 0; // Les mercenaries n'ajoutent pas de slots, ils en utilisent
    }
...
function normalizeIpfsValue(value) {
    ...
    // Strip ipfs:// prefix
    return value.replace(/^ipfs:\/\//i, '');
}

function buildIpfsUrl(cid, gatewayIndex) {
    if (!cid) return null;
    const normalized = normalizeIpfsValue(cid);
    if (/^https?:\/\//i.test(normalized)) return normalized;
    const index = Number.isInteger(gatewayIndex) ? gatewayIndex : 0;
    const base = IPFS_GATEWAYS[Math.max(0, Math.min(IPFS_GATEWAYS.length - 1, index))];
    return base + normalized;
}

// Global fallback handler to rotate gateways on error
window.fallbackIpfsImage = function fallbackIpfsImage(imgEl) {
    const cid = imgEl.getAttribute('data-cid');
    if (!cid) {
        imgEl.src = '../images/default-card.png';
        return;
    }
    const nextIndex = (parseInt(imgEl.getAttribute('data-gw-index') || '0', 10) + 1);
    if (nextIndex >= IPFS_GATEWAYS.length) {
        imgEl.src = '../images/default-card.png';
        return;
    }
    imgEl.setAttribute('data-gw-index', String(nextIndex));
    const nextUrl = buildIpfsUrl(cid, nextIndex);
    imgEl.src = nextUrl;
};
function displayTemplates(templateDetails, userTemplates) {
    const categories = ['crews', 'faces', 'arms', 'lands', 'warlords', 'mercenaries'];
        ...
        const cards = Object.keys(items).map(templateId => {
            ...
            if (detail) {
                ...
                if (category === 'crews') {
                    ...
                    statsHtml = `
                        <p><img class="image-stat-detail" src="../images/crew.png" alt="logo crew">Slots: ${stats.perCard.slots} (Total: ${stats.slots})</p>
                        <p>Rarity: ${stats.perCard.rarity}</p>
                    `;
                } else if (category === 'mercenaries') {
                    statsHtml = `
                        <p><img class="image-stat-detail" src="../images/Attack.png" alt="logo attaque">Attack: ${stats.perCard.attack} (Total: ${stats.totalAttack})</p>
                ...
                const cid = detail.img || detail.backimg || '';
                const initialUrl = buildIpfsUrl(cid, 0) || '../images/default-card.png';
                return `
                    <div class="col-4">
                        <div class="template-card">
                            ...
                            <div class="template-img">
                                <img 
                                    ...
                                    data-cid="${cid || ''}"
                                    data-gw-index="0"
                                    onerror="window.fallbackIpfsImage(this)"
                                >
                            </div>
...
function displayTotals(templateDetails, userTemplates) {
        ...
        Object.keys(items).forEach(templateId => {
            ...
            if (detail) {
                ...
                if (category === 'crews') {
                    ...
                    totals.warlords.slots += stats.slots;
                    totals.warlords.rarity[detail.rarity] = (totals.warlords.rarity[detail.rarity] || 0) + quantity;
                } else if (category === 'mercenaries') {
                    totals.mercenaries.attack += stats.totalAttack;
                    totals.mercenaries.defense += stats.totalDefense;
                    totals.mercenaries.moveCost += stats.totalMoveCost;
                    totals.mercenaries.rarity[detail.rarity] = (totals.mercenaries.rarity[detail.rarity] || 0) + quantity;
                }
            }
        });
    });
    ...
    sectionContainer.innerHTML = `
        <div class="totals-container d-flex justify-content-center flex-wrap">
            ...
            <div class="card lato">
                <h3 class="category-heading ethno">Warlords</h3>
                <ul>
                    <li>Total Slots: <span class="total-point">${totals.warlords.slots}</span></li>
                    <li>Total Rarity: <span class="total-point">${Object.entries(totals.warlords.rarity).map(([rarity, total]) => `${rarity}: ${total}`).join(', ')}</span></li>
                </ul>
            </div>
            <div class="card lato">
                <h3 class="category-heading ethno">Mercenaries</h3>
                <ul>
                    <li>Total Attack: <span class="total-point">${totals.mercenaries.attack}</span></li>
                    <li>Total Defense: <span class="total-point">${totals.mercenaries.defense}</span></li>
                    <li>Total Move Cost: <span class="total-point">${totals.mercenaries.moveCost}</span></li>
                    <li>Total Rarity: <span class="total-point">${Object.entries(totals.mercenaries.rarity).map(([rarity, total]) => `${rarity}: ${total}`).join(', ')}</span></li>
                </ul>
            </div>
        </div>
    `;
