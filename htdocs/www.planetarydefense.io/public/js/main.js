...
async function fetchLatestPayouts() {
    ...
    if (day >= 1) {
        ...
        const beforeDatePrev16 = `${prevYear}-${prevMonth.toString().padStart(2, '0')}-16T03:00:00.000Z`;
    ...
    if (day >= 16) {
        ...
        const beforeDateCurrent16 = `${year}-${month.toString().padStart(2, '0')}-16T03:00:00.000Z`;
    ...
    const beforeDateCurrent01 = `${year}-${month.toString().padStart(2, '0')}-01T03:00:00.000Z`;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        // Récupérer l'avatar - Utiliser une image par défaut en cas d'échec des API
        try {
            // Essayer plusieurs API en cas d'échec
            let assetResponse;
            const apiUrls = [
                `https://atomic-wax.tacocrypto.io/atomicassets/v1/assets/${playerAvatar}`,
                `https://atomic.3dkrender.com/atomicassets/v1/assets/${playerAvatar}`,
                `https://atomic-api.wax.cryptolions.io/atomicassets/v1/assets/${playerAvatar}`,
                `http://wax-atomic-api.eosphere.io/atomicassets/v1/assets/${playerAvatar}`,
                `http://atomic.wax.eosrio.io/atomicassets/v1/assets/${playerAvatar}`,
                `https://wax-aa.eu.eosamsterdam.net/atomicassets/v1/assets/${playerAvatar}`,
                `https://aa.dapplica.io/atomicassets/v1/assets/${playerAvatar}`,
                `https://atomic-wax.a-dex.xyz/atomicassets/v1/assets/${playerAvatar}`,
                `https://wax-atomic.alcor.exchange/atomicassets/v1/assets/${playerAvatar}`,
                `https://aa-wax-public1.neftyblocks.com/atomicassets/v1/assets/${playerAvatar}`,
                `https://wax.eosusa.io/atomicassets/v1/assets/${playerAvatar}`,
                `https://atomicassets.ledgerwise.io/atomicassets/v1/assets/${playerAvatar}`,
                `https://atomic-wax.qaraqol.com/atomicassets/v1/assets/${playerAvatar}`,
                `https://wax-atomic.eosiomadrid.io/atomicassets/v1/assets/${playerAvatar}`
            ];
            
            for (const apiUrl of apiUrls) {
                try {
                    assetResponse = await $.ajax({
                        url: apiUrl,
                        type: 'GET',
            ...
            // Si toutes les API échouent, utiliser l'image par défaut
            if (!assetResponse || !assetResponse.data) {
                console.warn('Toutes les API d\'avatar ont échoué, utilisation de l\'image par défaut');
                document.getElementById("avatarImage").src = "../images/20241021_214433_0000.png";
