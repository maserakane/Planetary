...
async function getDecay(user) {
  try {
    const response = await fetch('https://wax.cryptolions.io/v1/chain/get_table_rows', {
      ...
      body: JSON.stringify({
        code: 'dao.worlds', // Le contrat intelligent (smart contract) EOSIO
        table: 'votes', // La table que vous souhaitez interroger
        scope: 'magor', // La portée de la table
        json: true, // Demande des résultats au format JSON
        limit: 100, // Limite le nombre de résultats retournés
        lower_bound: user, // Clé de début (facultatif)
        upper_bound: user, // Clé de fin (facultatif)
      }),
    });
...
async function reward(landid) {
  try {
    ...
    if (chestdata && chestdata.rows && chestdata.rows.length > 0) {
      ...
      let voteTimeStamp = new Date(); // Utiliser l'heure actuelle comme valeur par défaut
      try {
        const decayResponse = await getDecay(owner);
